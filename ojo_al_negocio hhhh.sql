...
--
-- Modificado: se agrega columna `id_usuario_propietario` en `negocio`
-- y se agrega la tabla `favorito` (relación usuario <-> negocio)
...
-- para evitar el error #1046 - Base de datos no seleccionada
CREATE DATABASE IF NOT EXISTS `ojo_al_negocio` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `ojo_al_negocio`;
...
-- (NUEVA: relación de negocios favoritos por usuario)
--
...
-- (MODIFICADA: se agrega `id_usuario_propietario`)
--

CREATE TABLE `negocio` (
  `id_negocio` int(10) UNSIGNED NOT NULL,
  `id_usuario_propietario` int(10) UNSIGNED DEFAULT NULL,
...
ALTER TABLE `favorito`
  ...
  ADD KEY `fk_favorito_negocio` (`id_negocio`);
...
ALTER TABLE `negocio`
  ...
  ADD KEY `fk_negocio_usuario` (`id_usuario_propietario`);
...
ALTER TABLE `favorito`
  ...
  ADD CONSTRAINT `fk_favorito_negocio` FOREIGN KEY (`id_negocio`) REFERENCES `negocio` (`id_negocio`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Filtros para la tabla `negocio`
--
ALTER TABLE `negocio`
  ADD CONSTRAINT `fk_negocio_usuario` FOREIGN KEY (`id_usuario_propietario`) REFERENCES `usuario` (`id_usuario`) ON DELETE SET NULL ON UPDATE CASCADE;
